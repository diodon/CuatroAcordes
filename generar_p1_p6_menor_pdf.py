#!/usr/bin/env python3
"""Genera p1_p6_menor.pdf — Progresión 1 y Progresión 6 (Ángel Martínez),
   lado a lado, para los 12 tonos de la escala menor."""
...
OUT_PDF  = '/home/eklein/Documents/Cuatro/00-Clases/Practicas/p1_p6_menor.pdf'
...
QUALITY_TO_SUFFIX = {'maj': 'maj', '7': '7', 'min': 'm'}

# Etiquetas de columna (función armónica, minúscula = calidad menor)
P1_LABELS = ['V7', 'i']
P6_LABELS = ['ivm', 'i', 'V7', 'i']
N_P1 = len(P1_LABELS)
N_P6 = len(P6_LABELS)
N_COLS = N_P1 + N_P6

# ── Estilo (color distinto del de p1_p6_mayor, por indicación explícita —
#    índigo/azul en vez de ámbar, para diferenciar mayor/menor a simple vista) ─
HDR_BG      = colors.HexColor('#2A3A7A')
HDR_TEXT    = colors.white
TITLE_CLR   = colors.HexColor('#2A3A7A')
...
ROW_COLORS  = ['#FFFFFF', '#E8ECFB'] * 6   # alternating white / light indigo
...
def make_img_reader(root_semi, quality, chords):
    ...
    name = HYBRID_NOTES[root_semi % 12] + sym
...
def main():
    ...
    c.setTitle('Progresión 1 y Progresión 6 — Escala Menor')
    c.setAuthor('E. Klein')
    ...
    c.drawCentredString(PAGE_W / 2, subtitle_y,
                        'Escala Menor · V7–i · ivm–i–V7–i  (Prof. Ángel Martínez)')
    ...
    # ── Encabezado de columnas (V7, i, ivm, i, V7, i) ────────────────────────
    c.setFillColor(colors.HexColor('#4A5AA0'))
    ...
    c.setStrokeColor(colors.HexColor('#1A2A5A'))
    c.setLineWidth(0.4)
    ...
    for row_idx, root_es in enumerate(NOMBRES_ES):
        ...
        # Acordes de P1 y P6
        p1_chords = prog_p1(root_semi, 'minor')
        p6_chords = prog_p6(root_semi, 'minor')
